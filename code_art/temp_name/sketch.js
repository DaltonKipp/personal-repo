...
const GRID_SCALE = 40; // Sets grid scale (increase for larger grid size)
const RAD_SCALE = 3.0; // Circle radius scale
const X_SCALE = 1.5; // X speed scale
...
class Circle {
  constructor(x, y, circleRadius, circleColor) {
    ...
    this.speedX = X_SCALE * random(0.5,1); // Random speed in X direction
  ...
  update() {
    ...
    this.x += this.speedX
    this.y = windowHeight/2+200*sin(this.x/200) + 200*sin(this.x/100)
    ...
    if (this.x > windowWidth) {
      this.x = -this.circleRadius; // Reverses Direction
    }    
...
function generateCircles(cols, rows) {
  for (let x = 0; x < cols - 1; x++) { // For Every Column
      for (let y = 0; y < rows - 1; y++) { // For Every Row
          ...
          Circles.push(new Circle(x * GRID_SCALE, windowHeight/2, circleRadius, circleColor)); // Push New Circle
        }
      }
    }
...
function getColorBasedOnNoise(x, y) {
  ...
  if (noiseValue <= 80) {
    ...
    return [0, 150, 150, ALPHA]; // Cyan (Dark)
