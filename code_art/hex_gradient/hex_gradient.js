...
class Hexagon {
	constructor(x, y, r) {
		...
		// Calculates base perlin noise value
		this.baseNoiseValue = noise(this.x + this.noiseOffset, this.y);
	}
	
  // Renders Hexagon
	color() {
		// Calculate color based on precomputed values
		let noiseValue = noise(this.x + this.noiseOffset, this.y + this.noiseOffset);
		let RED = 500 * noiseValue * (1 - this.colorFactor);
		let GREEN = 500 * noiseValue * (1 - this.colorFactor);
